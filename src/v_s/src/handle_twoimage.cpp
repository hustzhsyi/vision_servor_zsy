...
//对原始图像进行处理（ushort转化成double类型，并把单位换成m,然后进行高斯滤波）
...
void calculate_XY_c(const Mat &picture_depth,const Mat & picture_desired ,vector<vector<double> >& X_c,vector<vector<double> >& Y_c)
{
...
//根据X=xZ计算图像平面中的坐标(单位为长度)(X=xZ,Y=yZ)
...
void image_Callback(const sensor_msgs::ImageConstPtr& msg)
{
  //获得深度图的desired and actual数据
    cv::Mat desired_image_sub;//,desired_image_sub_equal;
    desired_image_sub = cv_bridge::toCvShare(msg)->image;
   // cv::Mat desired_image_sub_equal(desired_image_sub.size(),desired_image_sub.type());
   // equalizeHist(desired_image_sub,desired_image_sub_equal);

    cv::Mat depth_image_sub;//,depth_image_sub_equal;
    depth_image_sub = cv_bridge::toCvShare(msg)->image;
   // equalizeHist(depth_image_sub,depth_image_sub_equal);

    //对深度图进行直方图均衡化后的灰度图
     cv::Mat grey_img,grey_equ_img;
      cv_bridge::CvImageConstPtr cv_ptr;
      //转换ROS图像消息到opencv图像,深度图的编码的方式有：TYPE_8UC1 TYPE_64FC1 ...
      ...
      imshow("Show depth-greyImage", grey_equ_img);
    ...
    if(get_desired)
    {
      //获得理想位姿下的图
       ...
       GaussianBlur(desired_image,desired_image,Size(3,3),0,0);
      get_desired = false;
  ...
  if (get_actual_depth)
  {
     ...
     // waitKey(0);
      ...
      GaussianBlur(depth_image,depth_image,Size(3,3),0,0);
...
//从相机的速度转换到末端的速度
     ...
     tool_vel(v,tfBuffer,vel_pub,1.5);
     getdiffimage=true;
...
/*
           ...
           velcam.linear.y = v(1);
           velcam.linear.z = v(2);
           velcam.angular.x = v(3);
           velcam.angular.y = v(4);
           velcam.angular.z = v(5);
